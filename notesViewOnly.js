mainApp.controller('NotesViewController', function($location) {
	var self = this;
	/*var items = [];
    ...
    self.alertOthers = function (currentNote) {
        var user = JSON.parse(localStorage.getItem('notes')).filter(function(element) {
            return element.noteTitle == currentNote;
        });
        document.getElementById('temp-msg').innerHTML = currentNote + " by " + user[0].currentUser;
        document.getElementById('note-components').style.visibility = 'visible';
